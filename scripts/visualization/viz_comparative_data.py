...
from comfi_examples.urdf_utils import build_human_model, load_robot_panda, lock_joints
...
class Paths:
    ...
    def from_args(
        ...
        q_swika_csv = root / "mocap" / split / subject_id / task / "q_cosmik_swika.csv"
        if not q_swika_csv.exists():
            raise FileNotFoundError(
                f"Missing joint angles mocap {split} CSV: {q_swika_csv}"
            )
...
class Scene:
    ...
    viz_human_cosmik: MeshcatVisualizer
    viz_robot: MeshcatVisualizer
    human_model: pin.Model
    human_data: pin.Data
    human_model_cosmik: pin.Model
    human_data_cosmik: pin.Data
...
def define_scene(
    ...
    model_h2, coll_h2, vis_h2, _ = build_human_model(urdf_path, urdf_meshes_path)
    data_h2 = model_h2.createData()

    # make visuals gray
    make_visuals_gray(vis_h2, rgba=(0.0, 1.0, 0.0, 0.7))
    ...
    model_h2, coll_h2, vis_h2, data_h2 = lock_joints(model_h2, coll_h2, vis_h2, joints_to_lock)
    ...
    viz_human_cosmik = MeshcatVisualizer(model_h2, coll_h2, vis_h2)
    viz_human_cosmik.initViewer(viewer)
    viz_human_cosmik.loadViewerModel("human_cosmik")
    viz_human_cosmik.display(pin.neutral(model_h2))
    ...
    return Scene(
        viewer=viewer,
        viz_human=viz_human,
        viz_human_cosmik=viz_human_cosmik,
        viz_robot=viz_robot,
        human_model=model_h,
        human_data=data_h,
        human_model_cosmik=model_h2,
        human_data_cosmik=data_h2,
        robot_model=model_r,
        robot_data=data_r,
    )
...
def main():
    ...
    if args.save_video:
        ...
        Paths.video = (
            ...
            / args.subject_id
            / args.task
            / "viz_all_data_comparison.mp4"
        )

    # read all data
    payload = load_all_data_comparison(paths, start_sample=0, converter=1000.0)
    q_ref = payload["q_ref"]
    q_swika = payload["q_swika"]
